...
\begin{document}
    ...
    A set $S \subseteq \N$ is said to be \tbf{ultimately periodic} if there are integers $X \in \N$ and $Y > 0$ such that $$\forall k \ge X \quad k \in S \iff k + Y \in S$$ In other words, after $N$ values --- starting from 0 --- the membership in $S$ repeats every $p$ numbers. A well-known fact in the theory of context-free languages is that a \tit{unary language} $L = \{1^n \mid n \in \N\}$ is regular if and only if the set $\{n \in \N \mid 1^n \in L\}$ is \tit{ultimately periodic}. In other words, any regular unary language is represented by a regular expression of the form $$1^X \rbk{1^Y}^*$$
    ...
    However, PDL with test \tit{can} distinguish the worlds, because it possible to build programs which \tit{depend on the truthness of propositions itself}. For instance, the program $(P?; A)^*; \lnot P?$ proceeds through $A$-edges as long as $P$ is true in the current state, and stops as soon as it becomes false. Thus, because test operators can observe the truth of propositions \tit{mid-program}, it is possible to express properties that depend on non-periodic features of the model, something that test-free PDL cannot capture.
