    ...
    \solution{
        ...
        \begin{itemize}
            ...
            \item \tit{relative complement}: fix a set $X \in A$; then, by definition of $A$ we trivially have that
                ...
                \end{itemize}
            \item \tit{modal operations}: fix two sets $X, Y \in A$; by definition, we have that
                \begin{equation*}
                    \begin{split}
                        m_<(X, Y) &= \{n \in \N \mid \exists x \in X, y \in Y \quad R_<(n, x, y)\} \\
                                  &= \{n \in \N \mid \exists x \in X, y \in Y \quad n < x \land n < y\}
                    \end{split}
                \end{equation*}
                Therefore, we have that

                \begin{itemize}
                    \item if both $X$ and $Y$ are finite, then
                        \begin{equation*}
                            \begin{split}
                                m_<(X, Y) &= \{n \in \N \mid n < \max(X), \max(Y)\} \\
                                          &= \{n \in \N \mid n < \min(\max(X), \max(Y))\}
                            \end{split}
                        \end{equation*}
                        which implies that $m_<(X, Y)$ is an \curlyquotes{initial segment of $\N$}, thus it is finite, hence $m_<(X, Y) \in A$
                    \item without loss of generality, if $X$ is finite and $Y$ is co-finite, then $\max(Y)$ is not defined, thus $$m_<(X, Y) = \{n \in \N \mid n < \max(X)\}$$ which implies that $m_<(X, Y)$ is still finite, hence $m_<(X, Y) \in A$
                    \item if both $X$ and $Y$ are co-finite, then neither $\max(X)$ nor $\max(Y)$ are defined, therefore $$m_<(X, Y) = \N$$ which implies that $\N - m_<(X, Y) = \N - \N = \varnothing$ meaning that $m_<(X, Y)$ is co-finite, hence $m_<(X, Y) \in A$
                \end{itemize}
        \end{itemize}
    }
        ...
        \begin{equation*}
            \begin{alignedat}{2}
                ...
                34. \vdash & \lnot \Box(\lnot p \lor \lnot q) \to \lnot \Box \lnot (p \land q) && \quad \quad (\mbox{MP(31, 33)}) \\
                           & \ldots && \\
                39. \vdash & (\Box p \land \lnot \Box \lnot q) \to \lnot \Box \lnot (p \land q) && \quad \quad (C(26, 34)) \\
                40. \lozenge p \leftrightarrow \lnot \Box \lnot p && \quad \quad (\mbox{K}) \\
            \end{alignedat}
        \end{equation*}
    }
