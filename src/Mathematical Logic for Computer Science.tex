...
\begin{document}
    ...
    Dynamic Logics are modal logics for representing the states and the events of dynamic systems. Their languages is both an assertion language able to express properties of computation states, and a programming language able to express properties of system transitions between these states.

    Pratt's \todo{who tf is pratt} original dynamic logic of programs was a \tit{first-order} modal logic, and \tbf{Propositional Dynamic Logic} (PDL) is the propositional counterpart of it. Being propositional, it makes no use of terms, predicates or functions, and its only two syntactic categories are \tit{propositions} and \tit{programs}.

    Therefore, to give meaning to statements in PDL we typically work with an abstract semantics in terms of \tbf{Labeled Transition Systems} (LTS), in which we label the transitions between the \tit{worlds} (i.e. the \tit{states} of the LTS) by names of \tit{atomic programs}. For instance, a transition labeled $\pi$ from one state $x$ to a state $y$ indicates that starting in $x$ there is a possible execution of the program $\pi$ that finishes in $y$. \tit{Possibility} and \tit{necessity} are expressed through modal operators that also indicate the programs they are referring to. In particular, the formula $\abk \pi \phi$ indicates that, starting from the \curlyquotes{current} state, there is a possible execution of the program $\pi$ that ends in a state in which $\phi$ is true. Analogously, the formula $\sbk \pi \phi$ means that, starting from the \curlyquotes{current} state, \tit{all} the possible executions of the program $\pi$ end in states in which $\phi$ is true.

    \section{Syntax}

    Now we are going to provide a more formal definition of the PDL version we will discuss. The syntax of the \curlyquotes{basic} PDL is based upon two countably infinite sets of symbols, namely $\Phi_0$ --- the set of \tit{atomic formulas} --- and $\Pi_0$ --- the set of \tit{atomic programs}.
    ...
    \begin{itemize}
        ...
        \item $\forall \phi \in \mathrm{Form}(\Phi_0) \quad \lnot \Phi_0 \in \mathrm{Form}(\Phi_0)$
        \item $\forall \phi, \psi \in \mathrm{Form}(\Phi_0) \quad \phi \lor \psi \in \mathrm{Form}(\Phi_0)$
        \item $\forall \alpha \in \mathrm{Prog}(\Pi_0), \psi \in \mathrm{Form}(\Phi_0) \quad \sbk \alpha \psi \in \mathrm{Form}(\Phi_0)$
    \end{itemize}

    where $\mathrm{Prog}(\Phi_0)$ is the set of \tit{more complex programs}, which is inductively as follows
    ...
    \begin{itemize}
        ...
        \item \tit{non-deterministic choice}: $(\alpha \cup \beta)$ is interpreted as \curlyquotes{do $\alpha$ followed by $\beta$, non-deterministically}
    ...
    \begin{itemize}
        ...
        \item $\abk{(\alpha ; \beta)^*}\phi$ menas that there is a s sequence of alternating executions of $\alpha$ and $\beta$ such that a state is reached where $\phi$ holds
    \end{itemize}

    As previously mentioned, the \tit{meaning} of PDL formulas and programs is interpreted over LTSs: in particular, we construct an LTS $\mathfrak M = (W, R, V)$ where
    ...
    For instance, consider the following LTS $\mathfrak = (W, R, V)$
    ...
    Given a model $\mathfrak M = (W, R, V)$, and a formula $\phi$, we say that $\mathfrak M, w \models \phi$ if and only if $w \in V(\phi)$. If this is true for all worlds $w \in W$, we say that $\phi$ is \tit{valid in $\mathfrak M$}, denoted as $$\mathfrak M \models \phi \iff \forall w \in W \quad \mathfrak M, w \models \phi$$ Lastly, we say that $\phi$ is \tit{valid} if it is valid in any model, i.e. $$\models \phi \iff \forall \mathfrak M \quad \mathfrak M \models \phi$$

    The purpose of the proof theory is to provide a \tit{characterization of validity} in terms of axioms and rules of inference. We are going to define a \tbf{deducibility predicate} $\vdash$ inductively by operations on formulas that depend only on their syntactic structure, in such a way that for all the formulas $\phi$ it holds that $$\vdash \phi \iff \models \phi$$

    As axioms, we assert that PDL contains every instance of the \tbf{distribution} axiom schema --- namely (K) --- and it is closed under the \tbf{necessitation} rule of inference --- or \tit{generalization} (N).
    \begin{equation*}
        \begin{alignedat}{2}
    ...
    We are now ready to define $\vdash$: first, we have that $$\vdash \phi \iff \varnothing \vdash \phi$$ in which case we say that $\phi$ is \tbf{$\vdash$-deducible}. Then, given a set of formulas $F$ and a formula $\phi$, we say that $\phi$ is \tbf{$\vdash$-deducible} from $F$ --- denoted as $F \vdash \phi$ if there exists a sequence $\phi_0, \ldots, \phi_n$ of formulas such that $\phi_n = \phi$, and for all $i \in [n]$ it holds that $\phi_i$ satisfies one of the following

    \begin{itemize}
        \item $\phi_i$ is an instance of an axiom schema
        \item $\phi_i$ is an instance of a formula of $\phi$
        \item $\phi_i$ comes from earlier formulas of the sequence by a rule of inference
    \end{itemize}

    \section{Completeness}

    In 1977 \textcite{segerberg} proposed an axiomatization of PDL which replaces (I) with the following fifth axiom --- the so called \tbf{induction} axiom schema $$(\mbox A 5) \quad \quad \phi \land \sbk{\alpha ^*} (\phi \to \sbk \alpha \phi) \to \sbk{\alpha^*} \phi$$ In fact, it is easy to prove that (I) can be $\vdash$-deduced from the other axioms:
    \begin{equation*}
        \begin{alignedat}{2}
        ...
        \end{alignedat}
    \end{equation*}
    The goal of Segerberg's work was to prove that such axiomatization of PDL was such that $\vdash$-deductions were \tbf{sound} and \tbf{complete}. \tit{Soundness} of $\vdash$ w.r.t. $\models$, which requires that $$\forall \phi \in \mathrm{Form}(\Phi_0) \quad \vdash \phi \implies \models \phi$$ can be proved by induction on the length of $\phi$'s deduction in $\vdash$.
    
    On the other hand, \tit{completeness} of $\vdash$ w.r.t. $\models$, which requires that $$\forall \phi \in \mathrm{Form}(\Phi_0) \quad \models \phi \implies \vdash \phi$$ was pursued by several logicians. Segerberg's work was the first attempt to prove the completeness of $\vdash$, however in 1978 he found a flaw in his argument. Then, in 1978 \textcite{parikh} published what is now considered the first proof of the completeness of $\vdash$, and since then various proofs have been published since, most notably the work by \textcite{kozen} of 1981.

    TODO \todo{vdash prime?}
