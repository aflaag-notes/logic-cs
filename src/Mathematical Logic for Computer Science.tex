...
\begin{document}
    ...
    \begin{itemize}
        ...
        \item $V(P) = W_m - \{w_{m - 1}, w_{2m - 1}, w_{2m}\}$
        \item $V(Q) = W_m$ for any other atomic formula $Q \in \Phi_0 - \{P\}$
    \end{itemize}
    ...
    We observe that this lemma follows from the fact that a unary language is regular if and only if it is \tit{ultimately periodic}.
    ...
    \begin{proof}
        ...
        \claim{
            ...
            We proceed by structural induction on $p$. If $p$ is a propositional variable, or $\top$, or $\bot$, then $n_A(p) = 0$ therefore for any $0 \le k \le m$ it holds that $$\mathfrak A_m, w_k \models p \iff \mathfrak A_m, w_{k + m} \models p$$ because $V(Q) = W_m$ for any $Q \in \Phi_0 - \{P\}$ by construction of $\mathfrak A_m$. Now, consider a formula $p$ different from the base cases, and let $m, k \in \N$ two integers satisfying the claim.
        ...
        By way of contradiction, suppose that there exists a test-free formula $p$ equivalent to $z$. Let $p' \in L_A$ be a formula that satisfies the previous lemma, and fix $m \ge 2$ and $k \ge 0$ to satisfy the conditions of the previous claim. Then, by the claim it holds that $$\mathfrak A_m, w_k \models p' \iff \mathfrak A_m, w_{k + m} \models p'$$ However, by definition of $\mathfrak A_m$ it holds that $\mathfrak A_m, w_0 \models z$ but $\mathfrak A_m, w_m \not\models z$, contradicting the fact that $z$ and $p$ were equivalent $\lightning$.
    \end{proof}
