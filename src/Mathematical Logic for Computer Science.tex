...
\begin{document}
    ...
    \solution{
        For all the subsequent propositions, we will assume that $\mathfrak M = (\N, \mathcal N, V)$ and $\mathfrak M' = (\mathbb B, \mathcal B, V)$ are two models.
                ...
                \begin{itemize}
                    \item By definition, for any $m \in \N$ it holds that
                        \begin{equation*}
                            ...
                            \end{split}
                        \end{equation*}
                        Now take $m = 1$, and suppose $V(p) = \{m\} = \{1\}$; then $m \in V(p) \iff \mathfrak M, m \models p$, however for instance $k = 0 \in \N$ is such that $m + 1 = 1 + 1 = 2 > 0 = k$ even though $0 \notin V(p)$, meaning that $\exists k \in \N \quad m + 1 > k \land k \notin V(p) \implies V(p) \neq \{k \in \N \mid m + 1 > k\} \iff \mathfrak M, m \not\models \lozenge_1 \Box_2 p$.
                \end{itemize}
        \end{enumerate}
    }
