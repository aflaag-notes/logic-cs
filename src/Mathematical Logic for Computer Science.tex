...
\begin{document}
    ...
    \tbf{Exercise 4.3} Let $\Sigma$ be a set of formulas in the basic modal language, and let $\textsf F$ denote the class of all frames. Show that $\Sigma \models_{\textsf F}^g \phi$ if and only if $\{\Box ^n \sigma \mid \sigma \in \Sigma, n \in \N\} \models_{\textsf F} \phi$.

    \solution{
        Let $\Pi := \{\Box ^n \sigma \mid \sigma \in \Sigma, n \in \N\}$. We are going to split the two directions of the proof into two claims.

        \claim{
            If $\Sigma \models_{\textsf F}^g \phi$, then $\Pi \models_{\textsf F} \phi$.
        }{
            ...
            \begin{equation*}
                \begin{alignedat}{2}
                    ...
                    \implies & \forall v \in W_w \quad \mathfrak M_w, v \models \phi \quad \quad \quad \quad (\Sigma \models_{\textsf F}^g \phi) && \\
            ...
            This proves that for any frame $\mathcal F \in \textsf F$ defined over a world $W$, any model $\mathfrak M$ over $\mathcal F$, and every world $w \in W$, it holds that $$\mathfrak M, w \models \Pi \implies \mathfrak M, w \models \phi$$ which implies that $\Pi \models_{\textsf F} \phi$ by definition.
        }

        \claim{
            If $\Pi \models_{\textsf F} \phi$, then $\Sigma \models_{\textsf F}^g \phi$.
        }{
            Assume that $\Pi \models_{\textsf F} \phi$; fix a frame $\mathcal F \in \textsf F$ defined over a world $W$, and a model $\mathfrak M$ over $\mathcal F$. Suppose that $\forall w \in W \quad \mathfrak M, w \models \Sigma$; then, by the claim of Exercise 2.2 we obtain the following
            \begin{equation*}
                \begin{split}
                    ...
                    \implies & \forall w \in W \quad \mathfrak M, w \models \phi \quad \quad \quad (\Pi \models_{\textsf F}\phi) \\
                \end{split}
            \end{equation*}
