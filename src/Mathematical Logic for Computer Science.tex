...
\begin{document}
    ...
    \solution{
        ...
        In particular, we observe that both of these sentences have rank 4, and this s not a conicidence. In fact we can show that the Duplicator can win the 4-rounds game, and by the Ehrenfeucht's theorem this directly implies that $\mathcal G_1 \cong_4 \mathcal G_2$.

        \begin{enumerate}
            \item On the first round, suppose that the Spoiler chooses the vertex 1; then, the Duplicator may answer with the vertex $a$
            \item On the second round, suppose that the Spoiler chooses the vertex 3; then, the Duplicator may answer with the vertex $c$
            \item On the third round, suppose that the Spoiler chooses the vertex 5; then, the Duplicator may answer with the vertex $d$, preserving the distances
            \item On the fourth round, suppose that the Spoiler chooses the vertex 4; then, the Duplicator must answer with the vertex $e$, to preserve the distances
            \item Finally, when the Spoiler chooses vertex 2, and the Duplicator is forced to answer with $b$, which cannot preserve the partial isomorphism --- for instance, $b$ is not adjacent with $e$ in $\mathcal G_2$, even though 2 and 4 are adjacent in $\mathcal G_1$
        \end{enumerate}
        
        Therefore, the duplicator wins the 4-round game, and the Spoiler wins the 5-round game.
    }
