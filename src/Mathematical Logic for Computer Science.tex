...
\begin{document}
    ...
    In particular, \tit{possibility} and \tit{necessity} are expressed through modal operators that also indicate the programs they are referring to. For instance, the formula $\abk \pi \phi$ indicates that, starting from the \curlyquotes{current} state, there is a possible execution of the program $\pi$ that ends in a state in which $\phi$ is true. Analogously, the formula $\sbk \pi \psi$ means that, starting from the \curlyquotes{current} state, \tit{all} the possible executions of the program $\pi$ end in states in which $\psi$ is true.
    ...
    \begin{itemize}
        ...
        \item \tit{test}: $\phi?$ is interpreted as \curlyquotes{proceed if $\phi$ is true, else fail}
    \end{itemize}
    ...
    To give meaning to statements in PDL we typically work with abstract semantics in terms of \tbf{Labeled Transition Systems} (LTS), in which we label the transitions between the \tit{worlds} (i.e. the \tit{states} of the LTS) by names of \tit{atomic programs}. For instance, a transition labeled $\pi$ from one state $x$ to a state $y$ indicates that starting in $x$ there is a possible execution of the program $\pi$ that finishes in $y$. Formally, consider an LTS $\mathfrak M = (W, R, V)$ where
    ...
    \begin{itemize}
        ...
        \item $(x, y) \in R(\alpha \cup \beta) \iff (x, y) \in R(\alpha) \cup R(\beta)$
    ...
    In the following section, we are going to present the most important details of their proof, but before going into the technical parts we will discuss the general idea. First, we observe that by removing tests, programs are restricted to \tit{regular expressions}, therefore each program defines a regular language.

    A set $S \subseteq \N$ is said to be \tbf{ultimately periodic} if there are integers $X \in \N$ and $Y > 0$ such that $$\forall k \ge X \quad k \in S \iff k + Y \in S$$ For instance, the following set is ultimately periodic $$S = \{0, 1, 2, 4, 6\} \cup \{k \ge 8 \mid \congmod{k}{0, 2}{3}\} = \{0, 1, 2, 4, 6, 8, 9, 11, 12, 14, 15, \ldots\}$$ in fact it holds that $$\forall k \ge 7 \quad k \in S \iff k + 3 \in S$$ hence $X = 7$ and $Y = 3$. Now, consider the following result of the theory of context-free languages.

    \tbf{Theorem.} A \tit{unary language} $L = \{1^n \mid n \in \N\}$ is regular if and only if the set $\{n \in \N \mid 1^n \in L\}$ is \tit{ultimately periodic}.

    \proofiff{
        Let $L = \{1^n \mid n \in \N\}$ be a regular unary language. In particular, since it is regular there exists a DFA $D = (Q, \{1\}, \delta, q_0, F)$ that recognizes $L$. Let $k = \abs Q$ be the number of states of $D$, and consider an input $1^n$ given as input to $D$: as the DFA reads the string, it follows a sequence of $n$ transitions over the $k$ states. Hence, by the pigeonhole principle, if $n > k$ the DFA \tit{must repeat} states when reading strings longer than $k$.

        This means that there is some value $X \ge 0$ and period $Y > 0$ such that the behaviour of $D$ on input $1^n$ for $n \ge X$ repeats with period $Y$, implying that $S = \{n \in \N \mid 1^n \in L \}$ must be ultimately periodic.
    }{
        Consider a unary language $L = \{1^n \in \N \mid n \in \N\}$, and suppose that the set $S= \{n \in \N \mid 1^n \in L\}$ is ultimately periodic. By definition, this implies that there are some $X \ge 0$ and $Y > 0$ such that $$\forall n \ge X \quad n \in S \iff n + Y \in S$$ We construct the following DFA $D = (Q, \{1\}, \delta, q_0, F)$:

        \begin{itemize}
            \item $Q = \{q_0, \ldots, q_{X + Y - 1}\}$
            \item $\forall i \in [0, X + Y - 1] \quad \delta(q_i, 1) = \soe{ll}{q_{i + 1} & i < X + Y - 2 \\ q_X & i = X + Y - 1}$
            \item $F = \{q_i \mid i < X \land i \in S\} \cup \{q_{X + r} \mid r \in [0, Y - 1] \land X + r \in S\}$
        \end{itemize}

        By construction, it is clear that $D$ recognizes $L$.
    }

    In other words, this result implies that any regular unary language is represented by a regular expression which ends in $$\rbk{1^Y}^*$$ for some $Y > 0$.

    This property is leveraged by Berman and Paterson as follows: the idea is to construct a family of models $\mathfrak A_m$ in which test-free PDL formulas \tit{cannot} distinguish the worlds in which we are performing the evaluation. To achieve this, they construct models $\mathfrak A_m$ for each $m \ge 2$ in which the only program present is $A$, hence the languages defined from the programs over $\mathfrak A_m$ are both regular and unary, therefore the set of the lengths of their strings must be ultimately periodic, i.e. the languages can be rewritten as a regular expression ending in $(A^n)^*$.

    Thus, they proceed to define each $\mathfrak A_m$ over a \tit{cyclical structure} made of $2m + 1$ worlds, where $2m + 1$ is a \tit{prime number}. This is a crucial detail, because it implies that the periodic part $(A^n)^*$ will generate \tit{all} the possible residues modulo $2m + 1$, meaning that each state is reachable starting from any possible state on the cycle, ensuring that no test-free PDL formula can distinguish the starting point of evaluation. This happens because ultimate periodicity forces the behaviour of formulas to eventually repeat regularly, hence test-free PDL formulas cannot \curlyquotes{count} beyond a bounded region.

    However, PDL with tests \tit{can} distinguish the worlds, because it is possible to build programs which \tit{depend on the truthness of propositions themselves}. For instance, the program $(P?; A)^*; \lnot P?$ proceeds through $A$-edges as long as $P$ is true in the current state, and stops as soon as it becomes false. Thus, because test operators can observe the truth of propositions \tit{mid-program}, it is possible to express properties that depend on non-periodic features of the model, something that test-free PDL cannot capture.

    Now that we presented the general idea of their counterexample, we are going to provide some technical details of their proof. Let $z$ be the formula $\abk{(P?; A)^*; \lnot P ? ; A; P ?}\top$. First, we need to introduce the definition of \tit{equivalence}.
    ...
    \centeredimage[The structure $\mathfrak A_m$ as depicted in the original paper.]{0.3}{../assets/Am.png}
    ...
    CPDL is the extension of PDL with the addition of the \tbf{converse}. For all programs $\alpha$, let $\alpha^{-1}$ stand for a new program such that $$(x, y) \in R \left(\alpha^{-1} \right) \iff (y, x) \in R(\alpha)$$ For instance, $\sbk{\alpha ^{-1}}\phi$ means that before executing $\alpha$, $\phi$ had to hold. To obtain a sound and complete system, the addition of these two axioms schemas suffices
    \begin{equation*}
        \begin{alignedat}{2}
    ...
    The operators defined over programs on standard PDL do not include \tbf{intersection}, but what happens when we add such operator? We obtain IPDL, in which for all programs $\alpha, \beta \in \mathrm{Prog}(\Pi_0)$ the expression $\alpha \cap \beta$ stands for a new program that has the following semantics $$(x, y) \in R(\alpha \cap \beta) \iff (x, y) \in R(\alpha) \cap R(\beta)$$ For instance, the intended reading of $\abk{\alpha \cap \beta} \phi$ is that if we execute $\alpha$ and $\beta$ in the \curlyquotes{current} state, there exists a state reachable by both programs which satisfies $\phi$. Thus, we have the following valid formula $$\models \abk{\alpha \cap \beta} \phi \to \abk \alpha \phi \land \abk \beta \phi$$ even though, in general, the converse is \tit{not} true.
